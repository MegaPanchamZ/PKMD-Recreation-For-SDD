...
class Generator():
    ...
    def gen_tiles_level(self):
        ...
        # [floor_array.append(row) for row in self.tiles_level]
        Rooms = (list(enumerate(self.room_list)))
        # print(Rooms)
        selection = random.randint(0, len(Rooms) - 1)
        coords = (Rooms[selection])[1]
        # print(coords)
        xPlus = random.randint(1, coords[2] - 1)
        yPlus = random.randint(1, coords[3] - 1)
        location = coords[0] + xPlus, coords[1] + yPlus
        # print(location, coords, xPlus, yPlus)
        # print(floor_array[location[1]])

        floor_array[location[1]] = change_char(floor_array[location[1]], location[0] - 1, "0")  # placeholder
...
def Create_FloorMap(size):
    ...
    if size == 'S':
        ...
        size = 96
        MaxRoom, MinSize, MaxSize = 15, 10, 14

    elif size == 'L':
        size = 144
        MaxRoom, MinSize, MaxSize = 20, 10, 15

    elif size == 'H':
        size = 216
        MaxRoom, MinSize, MaxSize = 20, 15, 20

    elif size == 'G':
        size = 324
        MaxRoom, MinSize, MaxSize = 25, 15, 40

    elif size == 'E':
        size = 486
        MaxRoom, MinSize, MaxSize = 50, 30, 60

    gen = Generator(size, size, MaxRoom, MinSize, MaxSize)
    gen.gen_level()
    gen.gen_tiles_level()
    return floor_array, location, roomList, corridorList
